...
public class PicScanInnerColorByClassCallBack implements CallBack {

    @Override
    public void call(Param paramContext, IdsService idsService, String path) {
        List<CarPhotoTestRowEntity> colorList = idsService.getRound5000SeriesSpecInnerColorPicRows();
    ...
    public <T> String buildUrl(Param paramContext, String path, T item) {
        ...
        int innerColorId = entity.getInnerColorId();
        ...
        String url = String.format(path + "&seriesid=%d&specid=%d&innerColorId=%d&classid=%d&picId=%d",
                seriesId, specId, innerColorId, classId, picId);
        return url;
    }
